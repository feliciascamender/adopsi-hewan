<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
        ...
        <input name="name" value="{{ old('name', $animal?->name) }}" class="form-input" required>
        @error('name')
    ...
    <div>
        ...
        <select name="species_id" class="form-input" required>
    ...
    <div>
        ...
        <input type="number" min="0" name="age_months" value="{{ old('age_months', $animal?->age_months ?? 0) }}" class="form-input" required>
        @error('age_months')
            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        ...
        <select name="status" class="form-input" required>
            @foreach(['available','pending','adopted'] as $status)
                <option value="{{ $status }}" @selected(old('status', $animal?->status ?? 'available') === $status)>
                    {{ ucfirst($status) }}
                </option>
            @endforeach
        </select>
